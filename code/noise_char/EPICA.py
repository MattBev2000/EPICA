...
class DmaAcquisition:
    ...
    def plot(self, acq_n, target_modules=["70"], name=None, show=True):
        figures = self.create_figures(acq_n, target_modules=target_modules, name=name)
        if show:
            plt.show()
        return figures

    def create_figures(self, acq_n, target_modules=["70"], name=None):
        ...
        for mod_id in target_modules:
            ...
            if any(mod_chans.values()):
                figures.extend(self._render_module_plots(mod_id, mod_chans, info, name=name))
        return figures
...
class EPICAFolderReader:
    ...
    def get_info(self, acq_n):
        """Returns the parsed .info metadata for an acquisition."""
        acq_code = f"{acq_n:05d}"
        all_files = os.listdir(self.data_dir)
        ...
        return self._parse_info(os.path.join(self.data_dir, info_file))

    def get_channel_shunt(self, acq_n, channel="CHA"):
        """Returns the configured current range for a channel, e.g. '1mA'."""
        info = self.get_info(acq_n)
        return info["shunt_a"] if channel.upper() == "CHA" else info["shunt_b"]
