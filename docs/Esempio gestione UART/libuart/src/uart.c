...
/**
 * \fn void configure_uart(uart_cfg * uart)
 * \brief Unconfigures uart connection (close fd)
 *
 * \param uart      The struct containing the uart configuration
 * 
 * \return none
 */
void unconfigure_uart(uart_cfg * uart)
{
    close(uart->fd);
}
...
/**
 * \fn uart_drive_control_line(uart_cfg * uart, unsigned int line, int level)
 * \brief drive RTS or DTR signals
 *
 * \param uart     The struct containing the uart configuration
 * \param line     The line to be driven (TIOCM_RTS o TIOCM_DTR)
 * \param level    The level to apply to the line
 * 
 * \return 0 if succeeded
 */
int uart_drive_control_line(uart_cfg * uart, unsigned int line, int level)
{
    int status;

    // #define TIOCM_DTR	0x002
    // #define TIOCM_RTS	0x004    
    if((line!=TIOCM_DTR) && (line!=TIOCM_RTS))
    {
      printf("Error, line not supported (0x%04X)\n", line);
      return -1;      
    }

    if (ioctl(uart->fd, TIOCMGET, &status) == -1) 
    {
      printf("Error %i from TIOCMGET: %s\n", errno, strerror(errno));
      return -2;
    }
    ...
    {
      printf("Error %i from TIOCMSET: %s\n", errno, strerror(errno));
      return -3;
    }
